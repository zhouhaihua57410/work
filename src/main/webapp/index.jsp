...
<script>
    ...
    function initLayPage(pageConf) {
        if (!pageConf) {
            ...
            pageConf.identity = $("#identity").val();
        }
    ...
    function fillTable(data, num) {
        ...
        $.each(data, function (index, obj) {
            ...
            if (userId.length !== 0 && obj.flag !== null && obj.flag.length !== 0) {
    ...
    function initLayPage2(pageConf) {
        if (!pageConf) {
            pageConf = {};
            let productName = $("#product_name").val();
            pageConf.pageSize = 12;
                ...
                page.render({
                    ...
                    jump: function (obj, first) {
                        if (!first) {
                            pageConf.currentPage = obj.curr;
                            pageConf.pageSize = obj.limit;
                            initLayPage2(pageConf);
                        }
                    }
                });
                fillTable2(data.list, (pageConf.currentPage - 1) * pageConf.pageSize); //页面填充
            })
        });
    }

    //填充表格数据
    function fillTable2(data, num) {
        let info = '';
        let userId = $("#userId").val();
        $.each(data, function (index, obj) {
            info += '<li class="product">'
